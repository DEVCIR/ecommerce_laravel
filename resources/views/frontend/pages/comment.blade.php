    ...
    <div class="display-comment"   @if($comment->parent_id != null) style="margin-left:40px;" @endif>
        <div class="comment-list">
            <div class="single-comment">
                @if($comment->user_info && $comment->user_info['photo'])
                ...
                <div class="content">
                    {{-- {{$post}} --}}
                    <h4>{{$comment->user_info['name'] ?? 'Unknown User'}} <span>At {{$comment->created_at->format('g: i a')}} On {{$comment->created_at->format('M d Y')}}</span></h4>
                    <p>{{$comment->comment}}</p>
                    @if($dep)
                        <div class="button">
